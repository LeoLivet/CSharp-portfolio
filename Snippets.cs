...
#region Challenge project - Create a Minigame
...
#region Exercise - Use the Visual Studio Code debugger to isolate and fix logic issues in a C# console application
...
while (transactions < 100)
{
    ...
    int customerPayment = paymentOnes + paymentFives * 5 + paymentTens * 10  + paymentTwenties *20;
    // display messages describing the current transaction
    Console.WriteLine("Transaction Number: " + transactions);
    Console.WriteLine($"Customer is making a ${itemCost} purchase");
    Console.WriteLine($"Customer Paid {customerPayment} dollars");
...
Console.WriteLine($"[{transactions}] transactions has been completed. The program will now end.");
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    //Temporarily holds the cash to check if register has enough change
    int cashTillTwenties = cashTill[3]; // 20s
    int cashTillTens = cashTill[2]; // 10s
    int cashTillFives = cashTill[1]; // 5s
    int cashTillOnes = cashTill[0]; // 1s
    ...
    if (changeNeeded > 0) //reverts the cashtill to before if not enough change has been provided
    {
        cashTill[3] = cashTillTwenties; // 20s
        cashTill[2] = cashTillTens; // 10s
        cashTill[1] = cashTillFives; // 5s
        cashTill[0] = cashTillOnes; // 1s
